...
- `config/totem.keymap` — the keymap (6 layers: `BASE`, `NAV`, `SYM`, `NUM`, `ADJ`, `FUN` — see "Editing the keymap" below). This overrides the default keymap bundled in the `zmk-keyboard-totem` module — ZMK prefers a `<shield>.keymap` found in `config/` over the shield's own.
...
- **FUN** — F1-F12, reachable only nested under NUM (see above). F1-F10 mirror NUM's number row exactly (same top-row positions as `1`-`0`); F11/F12 continue down the rightmost column — F11 at the home-row position under F10, F12 at the bottom-row position under that.
