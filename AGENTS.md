...
- `qmk/ergodox_ez/` — the QMK "keymap folder": `keymap.c` (five `LAYOUT_ergodox_pretty` layers, the `chordal_hold_layout` bilateral-mod guard, the `NUMF()` tap-digit/hold-F-key number row, nine combos gated to `_BASE` by `combo_should_trigger()`, and a `keyboard_post_init_user()` that pins the RGB mode), `rgb_matrix_user.inc` (the `LAYER_GLOW` effect — the only RGB effect built: a warm white on `_BASE` with a slowly drifting rainbow revealed under each pressed key, that rainbow itself at full brightness as `_GAME`'s color, and a pastel two-hue gradient on `_SYM`/`_NUM`/`_MOUSE`; colors, per-layer brightness and the `rainbow`/`reveal` flags live in its `layer_glow[]` table, indexed in `enum layers` order since the enum isn't visible there), `config.h` (tap-hold/combo tuning, and `#undef`s for every stock RGB animation the board enables), `rules.mk` (feature flags), `keymap.json` (`{"modules": ["zsa/oryx", "zsa/defaults"]}` — this, not an `ORYX_ENABLE` flag, is what keeps ZSA's Keymapp/live-training working on this firmware branch; `ORYX_ENABLE` doesn't exist here and is silently ignored if set).
