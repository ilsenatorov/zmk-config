...
Config for two personal keyboards. The **TOTEM** (38-key wireless split) runs **ZMK**; the **ErgoDox EZ Glow** (wired, ATmega32u4) runs **QMK** — ZMK has no AVR support, so it cannot run on the ErgoDox. The ErgoDox's `qmk/ergodox_ez/` keymap is its **own** layout — derived from the Oryx layout `xbvqK/nlyNZG` (BASE/GAME/SYM/NUM/MOUSE), not from the TOTEM's. What the two boards *do* share is typing feel: the same home-row mods, the same bilateral hold guard, and the same nine combos, added to the ErgoDox's base layer. See "The ErgoDox EZ (QMK)" below for that side; everything else in this file is TOTEM/ZMK.
...
- `.github/workflows/totem-ergodox-sync.yml` — CI: on any push/PR touching `config/totem.keymap`, warns (doesn't fail the build) if `qmk/ergodox_ez/keymap.c` wasn't touched in the same diff — a non-blocking nudge to check whether the change belongs on both boards. The two keymaps do **not** mirror each other layer-for-layer (see "The ErgoDox EZ (QMK)" below) — only the home-row-mod tuning and the combo set are meant to stay in sync, and there's no automated way to do that across the two formats.
...
**The ErgoDox runs its own layout, not a port of the TOTEM's.** `keymap.c` is derived from the Oryx layout `xbvqK/nlyNZG` — five layers, `_BASE`/`_GAME`/`_SYM`/`_NUM`/`_MOUSE`, with the ErgoDox's own numpad and mouse/RGB layers that the TOTEM has no equivalent for. Layers `_GAME` through `_MOUSE` are carried over from the Oryx source export verbatim, except that `_MOUSE`'s RGB controls were cut down to a single on/off key (`RM_TOGG`); `_BASE` was modified to add home-row mods and combos. The Oryx per-key RGB `ledmap` is **not** carried over — lighting is a custom effect (see below). When re-exporting from Oryx, diff against the export rather than rewriting by hand.

What *is* shared with the TOTEM, and what the sync workflow is nudging about: the home-row-mod flavour and timing, the bilateral hold guard, and the nine combos (`config/totem.keymap`'s `combos` block is the reference set — Caps Word, `_`, `-`, `:`, `?`, Home, End, `'`, `"`). Layer *design* changes on the TOTEM are expected to trip the warning without being ported; that's fine.

- `qmk/ergodox_ez/` — the QMK "keymap folder": `keymap.c` (five `LAYOUT_ergodox_pretty` layers, the `chordal_hold_layout` bilateral-mod guard, the `NUMF()` tap-digit/hold-F-key number row, nine combos gated to `_BASE` by `combo_should_trigger()`, and a `keyboard_post_init_user()` that pins the RGB mode), `rgb_matrix_user.inc` (the `LAYER_GLOW` effect — the only RGB effect built: a warm white on `_BASE` and a pastel two-hue gradient on `_SYM`/`_NUM`/`_MOUSE`, each with a slowly drifting rainbow revealed under every pressed key, and that rainbow itself at full brightness as `_GAME`'s color; colors, per-layer brightness and the `rainbow`/`reveal` flags live in its `layer_glow[]` table, indexed in `enum layers` order since the enum isn't visible there), `config.h` (tap-hold/combo tuning, and `#undef`s for every stock RGB animation the board enables), `rules.mk` (feature flags), `keymap.json` (`{"modules": ["zsa/oryx", "zsa/defaults"]}` — this, not an `ORYX_ENABLE` flag, is what keeps ZSA's Keymapp/live-training working on this firmware branch; `ORYX_ENABLE` doesn't exist here and is silently ignored if set).
...
- **Flash is the hard constraint on every future edit.** `LAYER_STATE_8BIT` caps layers at 8 (this keymap uses 5) and the ATmega32u4 has 32256 bytes total; the current keymap compiles to **23324 bytes — 8932 free (72%)**. That headroom exists only because `qmk/ergodox_ez/config.h` `#undef`s all ~43 stock RGB Matrix animations the upstream `keyboards/zsa/ergodox_ez/config.h` enables (with them the keymap sat at 31706 bytes, 550 free). Re-enabling animations is the fastest way back to a full chip. Check the build's printed size line after any feature change (`scripts/build-ergodox.sh` also greps and warns if free space drops under 512 bytes).
- `.github/workflows/draw-ergodox.yml` + `keymap-drawer/ergodox_ez.{yaml,svg}` — CI: on any push touching `qmk/ergodox_ez/keymap.c`, shallow-clones the ZSA fork (no submodules — those are only needed to *build* firmware, not to convert the keymap), runs `qmk c2json` to turn `keymap.c` into a QMK `keymap.json`, then `keymap-drawer` parses that into `ergodox_ez.yaml` and draws `ergodox_ez.svg`, committing both back. No reusable `draw-qmk.yml` exists upstream (unlike ZMK's `draw-zmk.yml`), so this workflow drives the CLI directly. **The `.yaml`/`.svg` are generated output — don't hand-edit them**; the intermediate `.json` is not committed. The physical layout is drawn from mainline QMK's `ergodox_ez/glow` (`-k ergodox_ez/glow` — passed explicitly since keymap-drawer's automatic QMK-keyboard lookup only knows mainline keyboard names, not the ZSA fork's `zsa/ergodox_ez/m32u4/glow` id used to parse the layers) since the physical layout is identical between the fork and mainline. To refresh locally, see the same `qmk c2json` + `keymap parse -q` + `keymap draw -k ergodox_ez/glow` steps as in the workflow file — needs Docker (`qmkfm/qmk_cli` image) or a local `qmk` install to run `c2json`.
...
- Bootloader entry: **the physical reset button** (paperclip hole, top-right of the right half). This keymap has no `QK_BOOT` key on any layer — neither did the Oryx layout it came from, so this is how the board has always been flashed. There is now flash to spare for one if wanted.
