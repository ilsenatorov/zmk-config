...
- `qmk/ergodox_ez/` — the QMK "keymap folder": `keymap.c` (five `LAYOUT_ergodox_pretty` layers, the `chordal_hold_layout` bilateral-mod guard, the `NUMF()` tap-digit/hold-F-key number row, nine combos gated to `_BASE` by `combo_should_trigger()`, the Oryx per-layer RGB `ledmap`, and `layer_state_set_user()` driving the three top-right status LEDs — one pattern per layer in Oryx's convention: BASE off, GAME LED1, SYM LED2, NUM LED3, MOUSE LED1+2; Oryx-generated firmware has this hook, so dropping it leaves the LEDs dark), `config.h` (tap-hold/combo tuning), `rules.mk` (feature flags), `keymap.json` (`{"modules": ["zsa/oryx", "zsa/defaults"]}` — this, not an `ORYX_ENABLE` flag, is what keeps ZSA's Keymapp/live-training working on this firmware branch; `ORYX_ENABLE` doesn't exist here and is silently ignored if set).
...
- Bootloader entry: **the physical reset button** (paperclip hole, top-right of the right half). This keymap has no `QK_BOOT` key on any layer — neither did the Oryx layout it came from, so this is how the board has always been flashed. With ~440 bytes of flash free there is no room to add one without trimming something first.
